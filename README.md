...
## Added Project Pipeline

This repo now contains a submission-oriented pipeline that follows the project
constraints:

- `main.py`: grader-compatible inference entrypoint. It loads `adapter/` or
  `checkpoint/` when present, runs local model inference, extracts JSON, and
  drops schema-invalid identifiers before writing predictions. If no model
  artifact exists yet, it falls back to a local train-split baseline for
  debugging only.
- `schema_linking.py`: prompt construction, schema serialization, JSON repair,
  schema validation, and the local fallback linker.
- `scripts/prepare_sft_data.py`: builds chat-format SFT JSONL files in
  `data/sft/`, including deterministic balancing of underrepresented databases.
- `scripts/train_sft.py`: plain TRL LoRA training path that saves a PEFT adapter
  to `adapter/`.
- `scripts/run_rapidfire_sft.py`: RapidFire AI grid search with 8 SFT configs
  over base model, LoRA target/rank, and learning rate.
- `scripts/run_validation.py`: runs `main.py` on the released validation input
  and evaluates with `eval.py`.

### Commands for the Current Setup

Prepare SFT data:

```bash
python scripts/prepare_sft_data.py --out_dir data/sft
```

Run the grader-compatible inference command:

```bash
python main.py --input validation_input.json --output validation_predictions.json
```
...
python eval.py \
    ...
    --schemas_dir schemas/ \
    --questions_input validation_input.json \
    --per_question_out per_question_validation.csv
```

Train a final LoRA adapter with TRL:

```bash
python scripts/train_sft.py \
    --train_file data/sft/train.jsonl \
    --eval_file data/sft/validation.jsonl \
    --base_model Qwen/Qwen2.5-1.5B-Instruct \
    --output_dir adapter
```

Run the RapidFire multi-config experiment:

```bash
rapidfireai start --tracking-backends mlflow
python scripts/run_rapidfire_sft.py
```

Do not fabricate `logs/` contents. After real RapidFire runs, copy or commit the
actual run artifacts/metrics generated by RapidFire into `logs/`, and report
only configs that were actually run.
